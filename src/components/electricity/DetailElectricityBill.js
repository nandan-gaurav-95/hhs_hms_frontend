...
const DetailElectricityBill = () => {
 ...
 const handleEleBillPdf = () => {
  ...
  doc.text("Tel : 080-22211356 / 22240309",145,42)
    ...
    doc.setFontSize(10); // Reset font size to normal
    doc.setFont("calibre", "normal");
    doc.text("1. Cheque / Drafts shall be drawn in favour of President/Secretary H.H.S & H.M.S., Bangalore", 21, 207); // Label "Name:"
    doc.text("2. Bill is to be paid in full. No part payment will be accepted.", 21, 212); // Label "Name:"
    doc.text("3. Please Check the correctness of R.R.No. Ledger No and amount in the receipt.", 21, 217);
    doc.text("4. Non-receipt of bill is not a valid reason for non-payment..", 21, 222);
    doc.text("5. (a) Electrical connection will be disconnected afte due date of every month.", 21, 227);
    doc.text("(b) The owner reserves the right of disconnection without assigning any reason.", 25, 232);
    doc.text("6. Payments, if made, after the due date will bear interest at 2% per month.", 21, 237);

    doc.rect(12, 241, 186, 20);
    doc.text("Fixed Charges    :                Rs. 240/- per KW or part there off the sanction load", 21, 247);
    doc.text("Energy Charges  :                Rs. 8.90 per unit", 21, 252);
    doc.text("D & R Charges   :                Rs. 25/- per Disconnection", 21, 257);
